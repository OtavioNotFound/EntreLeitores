...
export default function TrustedLending({ book, isOwned = false }) {
  ...
  async function solicitarEmprestimo(offer){const message=window.prompt('Escreva uma mensagem curta para o dono (opcional):','Olá! Gostaria de ler este livro.');if(message===null)return;setBusy(true);try{await requestBookLoan(user.id,offer.id,message);await load();toast('Pedido enviado. Combine a entrega somente após o aceite.');}catch(error){toast(error.message)}finally{setBusy(false)}}
  ...
  return <section className="emprestimos widget">
    ...
    <div className="emprestimos__lista">{offers.filter((offer)=>offer.owner_id!==user.id).map((offer)=>{const pedido=offer.loan_requests?.find((item)=>item.borrower_id===user.id);return <article key={offer.id}><div><strong>{offer.owner?.display_name||`@${offer.owner?.username}`}</strong><small>{offer.city} · {offer.audience==='followers'?'rede de confiança':'comunidade'}</small>{offer.notes&&<p>{offer.notes}</p>}</div><button className="btn-secundario" disabled={busy||Boolean(pedido)} onClick={()=>solicitarEmprestimo(offer)}>{pedido?statusLabel[pedido.status]:'Pedir emprestado'}</button></article>})}{!offers.length&&<small>Nenhum exemplar disponível ainda.</small>}</div>
  </section>;
}
