...
const kindLabels={highlight:'Destaque',reflection:'Reflexão',question:'Pergunta',vocabulary:'Vocabulário',citation:'Citação',favorite:'Favorito'};

export default function ReadingNotebook({bookId,currentProgress=0,onGoToPage,revision=0}){
  ...
  const [form,setForm]=useState({kind:'reflection',content:'',progress:currentProgress,pageNumber:'',chapter:'',color:'yellow'});
  ...
  useEffect(()=>{const timer=setTimeout(load,200);return()=>clearTimeout(timer)},[load,revision]);
  ...
  return <section className="caderno-leitura widget"><header><div><h2>Meu caderno privado</h2><p>Destaques e pensamentos visíveis somente para você.</p></div><button className="btn-secundario" onClick={()=>setOpen(!open)}>{open?'Cancelar':'+ Nova nota'}</button></header>
    ...
    <div className="caderno-leitura__lista">{notes.length?notes.map((note)=><article className={`caderno-leitura__nota caderno-leitura__nota--${note.color||'yellow'}`} key={note.id}><span>{kindLabels[note.kind]}</span><p>{note.content}</p><small>{note.progress!=null?`${note.progress}%`:''}{note.page_number?` · pág. ${note.page_number}`:''}{note.chapter?` · ${note.chapter}`:''}</small>{note.page_number&&<button className="caderno-leitura__ir" onClick={()=>onGoToPage?.(note.page_number)}>Ir para página</button>}<button aria-label="Excluir nota" onClick={()=>remove(note.id)}><DeleteIcon fontSize="small"/></button></article>):<p className="estado-vazio__texto">Seu caderno ainda está vazio.</p>}</div>
  </section>;
}
