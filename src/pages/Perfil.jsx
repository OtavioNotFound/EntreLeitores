    ...
    <section className="pagina ativa" id="pagina-perfil">
      ...
      {editing ? <form className="perfil-edicao widget" onSubmit={salvarPerfil}>
        <div className="perfil-edicao__imagens">
          ...
          <label className="perfil-imagem-campo perfil-imagem-campo--banner"><span>Banner do perfil</span><span className="perfil-imagem-preview perfil-imagem-preview--banner">{form.banner_url ? <img src={form.banner_url} alt="Prévia do banner" /> : 'Escolher banner'}</span><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(e) => escolherImagem(e, 'banner')} disabled={Boolean(uploading)} /><small>{uploading === 'banner' ? 'Enviando banner...' : 'JPG, PNG, WebP ou GIF · até 5 MB'}</small></label>
        </div>
