...
export default function Conquistas(){
  ...
  useEffect(()=>{let active=true;(async()=>{try{let [nextMetrics,nextSettings]=await Promise.all([getAchievementMetrics(user.id),getPlatformSettings()]);const calculated=calculateAchievements(nextMetrics);const inserted=await syncAutomaticAchievements(calculated);if(inserted)nextMetrics=await getAchievementMetrics(user.id);const xp=calculateAchievements(nextMetrics).filter((item)=>item.unlocked).reduce((sum,item)=>sum+item.xp,0);await syncReaderRank(xp);await refreshProfile();if(active){setMetrics(nextMetrics);setSettings(nextSettings);}}catch(error){if(active)toast(error.message);}})();return()=>{active=false};},[user.id]);
